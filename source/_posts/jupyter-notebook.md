...
data: 2020-05-28 21:21
categories: memo
mathjax: true
---
## 環境構築
[ANACONDA](https://www.anaconda.com/)をダウンロードします.
PythonやRとそのパッケージ(ライブラリ)を管理できます.
起動するとブラウザが開き, 次のような画面が表示されます.
{% asset_img 1.jpg %}
設定ファイル等作成されるので, 専用のフォルダに移動してから, 右上のNew/Python3をクリックしてファイルを新規作成します.
{% asset_img 2.jpg %}
すると次のような画面が開きます。ここで作業します.
{% asset_img tmp2.jpg %}
<br>
...
#### 5. plt.plot
plt.plot(x, y)でグラフが描けます.**xとyは配列もしくはジェネレータ**代入すると動きます.
よく使いそうなその他の引数をまとめました.
1. color(c)=: 線の色
英語の先頭のアルファベットを指定します.**黒:'k'**, 青:'b', 緑:'g', 赤:'r', 黄色:'c'(cyan), 紫:'m', 黄色:'y', 白:'w'
2. alpha=: 透過度
グラフの線の透過度を0?1で指定できます。1に近いほど濃くなります.
3. label=: 線の名前(label)を表示
必ずplt.legend()を同時に入力します.
4. linewidth(lw)=: 線の太さ
大きい値を入れるほど太くなります.
```Python
x = np.linspace(0, 2*math.pi, 1000)
...
#### 7. 操作にかかる時間を測る
操作の前に %timeitと打つと操作と同時にかかった時間を表示します.
